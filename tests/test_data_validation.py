...
class TestDatasetValidation:
    ...
    def test_has_technical_features(self, sample_processed_dataset):
        ...
        technical = ["rsi_14", "macd_histogram", "bollinger_width", "volume_trend", "price_to_52wk_high"]
    ...
    def test_no_nan_in_target(self, sample_processed_dataset):
        assert not sample_processed_dataset["target_return"].isna().any(), "target_return contains NaN values"
    ...
    def test_no_infinite_values(self, sample_processed_dataset):
        ...
        for col in numeric_cols:
            assert not np.isinf(sample_processed_dataset[col]).any(), f"Column {col} contains infinite values"

    def test_target_return_range(self, sample_processed_dataset):
        """Target returns should be between -100% and +100%."""
        assert (sample_processed_dataset["target_return"] >= -1.0).all(), "target_return below -100%"
        assert (sample_processed_dataset["target_return"] <= 1.0).all(), "target_return above +100%"

    def test_positive_revenue(self, sample_processed_dataset):
        if "revenue" in sample_processed_dataset.columns:
            assert (sample_processed_dataset["revenue"] > 0).all(), "Revenue should be positive"

    def test_minimum_sample_count(self, sample_processed_dataset):
        assert len(sample_processed_dataset) >= 10, f"Too few samples: {len(sample_processed_dataset)}"

    def test_multiple_companies(self, sample_processed_dataset):
        assert sample_processed_dataset["ticker"].nunique() >= 2, "Need at least 2 companies"
...
class TestPriceDataValidation:
    ...
    def test_datetime_index(self, sample_price_data):
        for ticker, df in sample_price_data.items():
            assert isinstance(df.index, pd.DatetimeIndex), f"{ticker} does not have DatetimeIndex"
...
class TestDataCleaningPipeline:
    ...
    def test_forward_fill_reduces_nans(self, sample_dirty_dataset):
        ...
        df["debt_to_equity"] = df.groupby("ticker")["debt_to_equity"].transform(lambda x: x.ffill())
