    ...
    for tag in tag_names:
        ...
        for entry in quarterly:
            ...
            if is_instant:
                ...
                if start:
                    days = (datetime.strptime(entry["end"], "%Y-%m-%d") - datetime.strptime(start, "%Y-%m-%d")).days
                    if 85 <= days <= 100:
                        values[end_date] = entry["val"]
...
quarters_list = [c["quarterly_income"].shape[1] for c in financial_data if not c["quarterly_income"].empty]
print(f"\nQuarters per Company:")
print(
    f"  Min: {min(quarters_list)}, Max: {max(quarters_list)}, "
