...
def parse_form4_xml(cik: int, accession_no: str, primary_doc: str) -> list[dict]:
    ...
    url = f"https://www.sec.gov/Archives/edgar/data/{cik}/" f"{accno_clean}/{primary_doc}"
    try:
        resp = _get(url)
    ...
    for txn in root.iter("nonDerivativeTransaction"):
        try:
            ...
            if code and shares > 0 and date:
                transactions.append({"date": date, "code": code, "shares": shares, "price": price})
...
def aggregate_to_quarters(transactions: list[dict]) -> pd.DataFrame:
    ...
    for qend, group in df.groupby("quarter_end"):
        ...
        rows.append({"quarter_end": qend, "net_direction": net_dir, "insider_buy_value": buy_value})
    ...
    for idx, ticker in enumerate(sorted(cik_map.keys()), 1):
        ...
        try:
            ...
            if not filings:
                ...
                print(f"{prefix}: {len(filings)} filings, " f"{n_buys}B/{n_sells}S transactions, {n_qtrs} quarters")
            ...
            failed += 1
            insider_trading[ticker] = pd.DataFrame(columns=["net_direction", "insider_buy_value"])
